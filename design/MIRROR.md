...
(the briefkist-app repo — formerly `app/` here) are built against. **Treat it
as a vendored-asset directory**: don't
...
> verbatim. Anything shipped from Briefkist — the native-app binaries
> (briefkist-app repo) and the `web/` fallback UI alike — **bundles fonts and
