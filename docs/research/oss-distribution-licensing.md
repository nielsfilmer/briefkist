...
- **OCR is the real porting problem, not Ollama.** Apple Vision (`pyobjc-framework-Vision`, already gated behind `sys_platform == 'darwin'` in `pyproject.toml`, with a clean engine abstraction + `available_engines()` in `/Users/nielsfilmer/code/my-flopy/spike/ocr_engines.py`) simply doesn't exist off macOS. Linux = PaddleOCR becomes the *primary*, so the Phase 0 benchmark delta between the two engines becomes user-facing honesty material ("expect X pp lower OCR accuracy on Linux" or the reverse).
